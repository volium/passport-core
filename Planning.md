...
- Coalesce overlapping readiness checks so navigation/visibility events do not queue repeated full-package checks or overwrite a newer download/delete result. Fully verify downloads/updates before activation and explicit rollback, but use lightweight reopening on startup/foreground: validate saved manifest identity, required chunk keys, both styles, and the PMTiles header. Do not rehash all payloads or schedule an automatic background scan. Keep IndexedDB; no storage-backend migration is part of this change.
- Foreground UX refinement: after the first inventory check, reconcile availability silently without publishing a transient Checking map state or disabling/collapsing controls. Repeat chunk-key presence checks to detect removal, but reuse successful header/style validation for the same immutable generation during this manager session. A new session or changed generation validates rendering metadata again. Genuine missing/unreadable storage still changes status immediately; this is not a periodic full integrity scan.
...
At startup and foreground refresh, validate the installed manifest and package identity, enumerate every required archive/resource chunk key without reading all payloads, and validate both styles and the PMTiles header. Only previously activated generations use this lightweight path; downloads/updates and explicit rollback retain full verification. No automatic background full scan is required. Missing keys invalidate availability; truncated/unreadable bytes invalidate the matching generation when accessed. Same-length corruption outside checked content may remain undetected, an accepted tradeoff for fast reopening. Read/parsing failures remain actionable, with explicit repair/redownload and independent renderer recovery. Do not silently use cached HTTP ranges as an installed package or erase passport records.
...
Show local feedback when opening the file chooser, while importing, on a no-selection result, and on success or failure. A no-selection result must not imply that the user deliberately cancelled. Each user-triggered attempt uses a fresh file input while preserving app state; device testing confirmed that fresh inputs do not fix the intermittent Chrome-on-iOS picker failure after export/save. It also reproduces without core on a standalone page, while the owner could not reproduce it in Safari. Preserve neutral no-selection feedback and advise saving unfinished visits before reload; do not add automatic retries, reloads, or speculative delays. See the feedback contract for the investigation outcome. Import completion stays in My passport even if the user switches tabs during the operation. Clear successful import and cancellation feedback after five seconds; retain actionable errors until retry. Handle file-access and storage failures without assuming private browsing is unsupported. Private-browser acceptance must include the actual device file chooser; automated file injection alone is insufficient.
...
25. **Offline map availability requires a fully verified installation and current lightweight checks of its manifest, required chunk keys, styles, and header; it does not require a full checksum scan at every launch. Cached ranges or a historical download flag alone are insufficient.**
