...
- Core: TypeScript package `@passport/core` 0.4.1, public models/API, program validation, viewport-height explorer and My passport panel, Leaflet map with selectable styles, synchronized selection and alias-aware filters, IndexedDB schema v1, date-only visits, notes/edit/delete, regional progress, and validated JSON restore. Optional airport reference fields support identifiers, addresses, runways, cautions, and dated source links.
...
- Maps: Leaflet + public OpenStreetMap raster tiles by default, with optional CARTO light/dark tiles using a dedicated Basemaps key. The worker does not cache or prefetch map tiles. Offline airport/passport functions are available after the production shell is cached; detailed offline basemaps are not promised. See app development notes for provider policy.
